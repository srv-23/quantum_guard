...
def parse_pcap(pcap_path):
    ...
    try:
        ...
        for pkt in cap:
            count += 1
            if count == 1:
                print(f"[DEBUG] First packet layers: {pkt.layers}")
            try:
                ...
                if 'IP' in pkt:
                    src_ip = pkt.ip.src
                    dst_ip = pkt.ip.dst
                elif 'IPv6' in pkt: # Graceful fallback if needed, though filter is "ip"
                    src_ip = pkt.ipv6.src
                    dst_ip = pkt.ipv6.dst
                else:
                    continue 
                ...
                if hasattr(pkt, 'tls'):
                    try:
                        # In JSON output, fields can be accessed as attributes
                        # Check recursively or specifically for the field if multiple layers exist
                        # But typically pkt.tls.record_content_type works for simple packets
                        # We might need to handle cases where it's not present or is a list
                        ...
                        if hasattr(pkt.tls, 'record_content_type'):
                            ...
                            if isinstance(val, str):
                                # Sometimes it's a comma separated string in XML/JSON text
                                # Or just a string "23"
                                if ',' in val:
                                ...
                                # Start of debug - maybe it's an object?
                                tls_content_type = [int(str(val))]

                    except Exception as e:
                        # Missing field or parse error
                        pass
...
def build_dataset(pcap_filename, dataset_name, label_logic):
    """
    ...
    if pcap_filename == "train_benign.pcap":
        ...
        # If alert_ratio > 0.0 (Benign has 0) OR handshake_ratio > 0.15 (Benign max ~0.11) -> 1, else 0
...
def main():
    ...
    build_dataset("train_benign.pcap", "train_benign.csv", None)
    
    # Process Test Mixed
    build_dataset("test_mixed.pcap", "test_mixed.csv", None)
